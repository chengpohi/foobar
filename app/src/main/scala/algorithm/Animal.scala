...
object Animal {
  def main(args: Array[String]): Unit = {
    val res: String = Source
      .fromFile(this.getClass.getResource("historyData.txt").getFile)
      .getLines()
      .toList
    ...
    res
      .map(r => {
        ...
        filter.length > 2 match {
          case true => {
            val animals: List[Option[Animal]] =
              filter.splitAt(2)._2.map(t => parseAnimal(t)).toList
  ...
  def parseAnimal(s: String): Option[Animal] = {
    s.split("\\s+") match {
      case Array(animal, lastX, lastY) =>
        Some(Animal(animal, lastX.toInt, lastY.toInt))
  ...
  case class Animal(animal: String,
                    lastX: Int,
                    lastY: Int,
                    xOffest: Option[Int] = Some(0),
                    yOffest: Option[Int] = Some(0))
  case class FixedTimeAnimals(id: String,
                              name: String,
                              animals: Option[List[Animal]] = None)
}
