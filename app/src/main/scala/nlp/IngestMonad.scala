...
case object connect {
  def to(url: String)(implicit timeout: Int) =
...
object IngestMonad {
  ...
  final case class IngestDocument(words: List[WordTerm],
                                  stopWords: List[WordTerm],
                                  size: Int) {
    def all: List[WordTerm] = words ++ stopWords
  }
  sealed trait Service[A]
  case class GetDoc(url: String, timeout: Option[Int] = Some(60 * 1000))
      extends Service[WebDoc]
...
object WordTokenizer {
  ...
  val STOP_WORDS =
    Source.fromURL(getClass.getResource("/stopwords.txt")).getLines().toSet
  def apply(text: String): (List[String], List[String], Int) = {
    val w = WORD_PATTERN
      .findAllIn(text)
      .matchData
      .map(i => i.group(0).trim.toLowerCase)
      .toList
