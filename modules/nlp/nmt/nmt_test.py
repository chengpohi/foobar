...
import os
dir_data = os.path.dirname(os.path.abspath(__file__)) + "/testdata/"
print(dir_data)
...
def _update_flags(flags, test_name):
  ...
  flags.src = "en"
  flags.tgt = "vi"
  flags.train_prefix = (dir_data +
                        "iwslt15.tst2013.100")
  flags.vocab_prefix = (dir_data +
                        "iwslt15.vocab.100")
  flags.dev_prefix = (dir_data +
                      "iwslt15.tst2013.100")
  flags.test_prefix = (dir_data +
                       "iwslt15.tst2013.100")
...
class NMTTest(tf.test.TestCase):
  ...
  def testTrain(self):
    ...
    nmt_train.add_arguments(nmt_parser)
    FLAGS, unparsed = nmt_parser.parse_known_args()

    _update_flags(FLAGS, "nmt_train_test")

    default_hparams = nmt_train.create_hparams(FLAGS)

    train_fn = train.train
    nmt_train.run_main(FLAGS, default_hparams, train_fn, None)
  ...
  def testInference(self):
    ...
    FLAGS.inference_input_file = (dir_data +
                                  "iwslt15.tst2013.100.en")
    FLAGS.inference_output_file = os.path.join(FLAGS.out_dir, "output")
    FLAGS.inference_ref_file = (dir_data +
                                "iwslt15.tst2013.100.vi")

    default_hparams = nmt_train.create_hparams(FLAGS)

    inference_fn = inference.inference
    nmt_train.run_main(FLAGS, default_hparams, None, inference_fn)
